...
// == MODIFIED FOR ARGOS ==========================================================================
		...
		cliSerial->printf_P(PSTR("Sonar %02X: %d cm  "), sonar->getAddress(), sonar->read());
		// Added for testing: REVERT TO ORIGINAL WHEN DONE
#ifdef SRF08_GETTIME
		cliSerial->printf_P(PSTR("%05d us  "), sonar->readTime);
#endif
#ifdef SRF08_GETECHOES
		cliSerial->printf_P(PSTR("\n"));
		for (uint8_t i=0; i<SRF08_GETECHOES; i++)
			cliSerial->printf_P(PSTR("      %02d: %03d %03d\n"),i,sonar->buff[2*i],sonar->buff[2*i+1]);
#else
		cliSerial->printf_P(PSTR("%s\n"),(sonar->healthy ? "Ok" : "Error"));
#endif
// ================================================================================================
